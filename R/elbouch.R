...
elbouch.test <- function(y, x = NULL){
  ...
  tstat = elbouch.statistic(y, x)
  names(tstat) <- "Z"

  pval = 2 * pnorm(q = tstat, lower.tail = FALSE)
...
elbouch.statistic <- function(y, x = NULL){
  ...
  if(!is.null(x)){
    ...
    S = var(y)
    S1 =  diag(2) / S
    B = sum( apply(cbind(y,rep(0, n + 1)), 1, function(x) t(x) %*% S1 %*% x )^2 ) / (n + 1)
    
    s1 = acf(y, lag.max = n, type = "covariance", plot = FALSE)$acf[1:n, , ]
    
    Q1 = 12 * sum(s1^2  * seq(n, 1) / S  ) / (n+1)^2  
    Q2 =  2 * sum(s1^4  * seq(n, 1) / S ^ 2) / (n+1) 
